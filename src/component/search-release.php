...
<input type="hidden" id="search-type-value" value="release"">
<div class="search-type">
<button onclick="searchSong()" class="search-type-btn" >Songs</button>
    <button onclick="searchRelease()" class="search-type-btn active-btn" disabled>Release Year</button>
...
<?php
    ...
    if ($search_query !== "%%"){
        $row_count_query = "SELECT COUNT(*) FROM song WHERE YEAR(tanggal_terbit) = ?";
        if ($filter_genre !== "all"){
            $row_count_query = "SELECT COUNT(*) FROM song WHERE YEAR(tanggal_terbit) = ? AND genre = '$filter_genre'";
        }
        ...
        $sql = "SELECT song_id, song.judul AS song_title, song.image_path, song.penyanyi, album.judul AS album_title, YEAR(song.tanggal_terbit), song.genre FROM song LEFT JOIN album ON song.album_id = album.album_id WHERE YEAR(song.tanggal_terbit) = ? ORDER BY $sort_by DESC LIMIT $song_per_page OFFSET $left_id_limit";
        if ($filter_genre !== "all"){
            $sql = "SELECT song_id, song.judul AS song_title, song.image_path, song.penyanyi, album.judul AS album_title, YEAR(song.tanggal_terbit), song.genre FROM song LEFT JOIN album ON song.album_id = album.album_id WHERE YEAR(song.tanggal_terbit) = ? AND song.genre = '$filter_genre' ORDER BY $sort_by DESC LIMIT $song_per_page OFFSET $left_id_limit";
        }
        $stmt = $db->prepare($sql);
        $stmt->bind_param("i", $search_query);
        $stmt->execute();
