...
<input type="hidden" id="search-type-value" value="song"">
<div class="search-type">
    <button onclick="searchSong()" class="search-type-btn active-btn" disabled>Songs</button>
...
<?php
    ...
    if ($search_query !== "%%"){
        $row_count_query = "SELECT COUNT(*) FROM song WHERE judul LIKE ?";
        if ($filter_genre !== "all"){
            $row_count_query = "SELECT COUNT(*) FROM song WHERE judul LIKE ? AND genre = '$filter_genre'";
        }
        ...
        $sql = "SELECT song_id, song.judul AS song_title, song.image_path, song.penyanyi, album.judul AS album_title, YEAR(song.tanggal_terbit), song.genre FROM song LEFT JOIN album ON song.album_id = album.album_id WHERE song.judul LIKE ? ORDER BY $sort_by DESC LIMIT $song_per_page OFFSET $left_id_limit";
        if ($filter_genre !== "all"){
            $sql = "SELECT song_id, song.judul AS song_title, song.image_path, song.penyanyi, album.judul AS album_title, YEAR(song.tanggal_terbit), song.genre FROM song LEFT JOIN album ON song.album_id = album.album_id WHERE song.judul LIKE ? AND song.genre = '$filter_genre' ORDER BY $sort_by DESC LIMIT $song_per_page OFFSET $left_id_limit";
        }
        $stmt = $db->prepare($sql);
        $stmt->bind_param("s", $search_query);
        $stmt->execute();
